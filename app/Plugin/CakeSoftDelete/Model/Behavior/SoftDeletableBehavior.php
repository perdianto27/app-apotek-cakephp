...
class SoftDeletableBehavior extends ModelBehavior {
     ...
     *
     * @param object $model    using the behavior
    ...
    protected function findConditionToExcludeDeleted($model) {
    ...
    public function softDelete($model, $id = null) {
        // Remember current id
    ...
    protected function markRecordAsDeleted($model) {
        switch ($this->getDeletedFieldColumnType($model)) {
            case 'boolean':
    ...
    public function softDeleteAll($model, $conditions, $cascade, $callbacks) {
     ...
     * @return void
     */
    public function includeDeletedRecords($model) {
     ...
     * @return void
     */
    public function excludeDeletedRecords($model) {
     ...
     */
    protected function checkForDeletedField($model, $conditions) {
        $field_name = $this->getFieldName($model);
        $model_field_name = $this->getModelFieldName($model);

        return ( isset($conditions[$field_name]) or isset($conditions[$model_field_name]) or isset($conditions['OR'][$field_name]) or isset($conditions['OR'][$model_field_name]) or isset($conditions['NOT'][$field_name]) or isset($conditions['NOT'][$model_field_name]));
    }

    protected function getFieldName($model) {
        return $this->settings[$model->alias]['field_name'];
    }

    protected function getModelFieldName($model) {
        $field_name = $this->getFieldName($model);
        return "{$model->alias}.$field_name";
    }

    protected function getDeletedFieldColumnType($model) {
