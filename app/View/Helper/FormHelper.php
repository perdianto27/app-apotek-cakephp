...
class FormHelper extends AppHelper {
     ...
     * @var array
     */
    public $helpers = array('Html', 'Access', 'Array');
    ...
    public function labelStatus($QuoteState) {
        switch ($QuoteState) {
            case '1':
                return 'default';
                break;
    ...
    public function postLink($title, $url = null, $options = array(), $confirmMessage = false) {
        ...
        $out .= $this->Html->link($title, $url, $options, false, true);
        return $out;
        //return '';
    }

    public function doPending($url = null, $id, $ps) {
        $html = '';
        ...
        if ($ps == 0) {
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="myModalLabel">Pending Order</h4>
                        </div>
                        <div class="modal-body">
                            <textarea style="width:95%;" rows="5" id="pending_note" placeholder="Input your reason"></textarea>
                            <div class="alert" id="pending-al">Note minimal 5 karakter</div>
                            <button type="button" id="pending_button" class="btn btn-primary">Save changes</button>
                        </div>
                    </div>
                </div>
            </div>';
            ...
            <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4>Are you sure to release order pending status?</h4>
                            <input type="hidden" id="url" name="url" value="' . $url . '" />
                            <input type="hidden" id="id" name="id" value="' . $id . '" />
                            <input type="hidden" id="ps" name="ps" value="' . $ps . '" />
                        </div>
                        <div class="modal-body" style="float:right;">
                            <button type="button" id="release_button" class="btn btn-primary">Lepas Pending</button>
        ...
        $var_string = ($ps == 0 ? 'Pending Order' : 'Lepas Pending');
        $html .= '<li style="margin-right:5px;"><a id="but_start" class="btn btn-default" data-toggle="modal" data-target="#myModal">' . $var_string . '</a></li>';
        //$html .= '<li style="margin-right:5px;">'.$this->postLink(__($var_string), array('action' => 'edit/12'), array('class' => 'btn btn-default', 'id' => 'but_start', 'data-toggle' => 'modal', 'data-target' => '#myModal'), false, false).'</li>';
        echo $html;
        print <<<DOPENDINGSCRIPT
<script>
    $('#release_button').click(function(){
        ...
        var new_ps = 0;
        if(ps == 0) new_ps = 1;
        else if(ps == 1) new_ps = 0;
        var releaseUrl = url + 'orders/doPendingOrder/' + id + '/' + new_ps + '/';
        var nextLocation = url + 'orders/edit/' + id + '/';
        $.ajax({
            type: 'POST',
            url: releaseUrl,
        }).done(function() {
    ...
    $('#pending_button').click(function(){
        var ncontent = $('#pending_note').val();
        var url = $('#url').val();
        var id = $('#id').val();
        ncontent = htmlEncode(ncontent);

        var postUrl = url + 'notes/add/' + id + '/';
        ...
        if(ncontent.length > 4){
            $.ajax({
                type: 'POST',
                url: postUrl,
                data: 'data[Note][content]=' + ncontent
            }).done(function() {
                $('#myModal').modal('hide');
                postUrl = url + 'orders/doPendingOrder/' + id + '/' + new_ps + '/';
                $.ajax({
                    type: 'POST',
                    url: postUrl,
                }).done(function() {
            ...
            $('#pending-al').attr('class', 'alert alert-danger');
        }
    });
</script>
DOPENDINGSCRIPT;
    ...
    public function postLinkSkipACL($title, $url = null, $options = array(), $confirmMessage = false) {
        return $this->postLink($title, $url, $options, $confirmMessage, true);
    }
    ...
    public function submit($caption = null, $options = array()) {
        ...
        $out .= '<h4 id="form_submit_button_fake" style="display:none">'
                . '<img src="/images/10.gif"/>Submitting...Please wait </h4>';
        $out .= '<script type="text/javascript">
                    $(function(){
                        $("#form_submit_button").click(function(event){                        
                            var parent_form_id=$(this).parents("form").attr("id");
                            $("#"+parent_form_id).parsley();
                            console.log(parent_form_id);                            
                            if($("#"+parent_form_id).parsley().isValid()==true){
                                $(this).css("display","none");
                                $("#form_submit_button_fake").css("display", "inherit");
                                setTimeout(function(){
                                    $("#form_submit_button").css("display", "inherit");
                                    $("#form_submit_button_fake").css("display", "none");
                                }, 50000);
                                return true;
                            }
                        });
    ...
    /*     * **
     * Special Input function that choose form input function
     * only if the current user has edit permission of the Order at order's
     * current state
     */

    public function inputCond($fieldName, $options = array()) {
        $state_id = $this->value('state_id');
        return $this->input($fieldName, $options);
//        if ($this->Access->hasOrderEditPermission($state_id)) {
//            return $this->input($fieldName, $options);
//        } else {
...
//        }
    ...
    public function nextState($ps) {
        if ($ps != 1) {
            $state_id = $this->value('State.id');
            //if ($this->Access->hasOrderEditPermission($state_id)) {
            $state_label = $this->value('State.next_button_label');
            $state_name = $this->value('State.name');
            $orderId = $this->value('Order.id');
            return $this->Html->LinkSkipACL($state_label
                            , array('controller' => $state_name, 'action' => 'base', $orderId));
            //}
        }
        return '';
    }
    ...
    /**
     *
     *   generate active Suppliers
     *
     *   @param array $suppliers array of suppliers
     *   @param array $buy
     */
    protected function generatedActiveSuppliers(array $suppliers, array $buy = NULL) {
        $results = '';
        foreach ($suppliers as $sup) {
            if ((int) $sup['Supplier']['status'] <= 0) {
                $_style = 'style="background-color:#DDD;" disabled="disabled"';
            } else
                $_style = NULL;

            $selected = ((int) $buy['supplier_id'] === (int) $sup['Supplier']['id']) ? 'selected="selected"' : '';
            $results .= '<option ' . $selected . ' value="' . $sup['Supplier']['id'] . '" ' . $_style . '>' . $sup['Supplier']['name'] . '</option>';
            unset($_style);
        }
    ...
    /**
     * Upgrade_order.ctp HTML
     * "sepecial function for handling openlead"
     */
    function openBuyHtml($suppliers, $showDiscount = false, $b = null) {
        $set = isset($b) && is_array($b);
        $order_buy = '
            <div class="order_buy">'
                . ($set ? '' : '<a href="#" class="remove_order_buy">x</a>')
                . ($set ? $this->input('Buy.id.', array('type' => 'hidden', 'value' => $b['id'])) : '')
                . $this->input('Buy.qty.', array(
                    'style' => 'width:60px;',
                    'class' => 'jm',
                    'type' => 'number',
                    'label' => 'Jumlah',
        ...
        $order_buy .= '<div class="input select required">'
                . '<label for="BuySupplierId">Supplier </label>'
                . '<select name="data[Buy][supplier_id][]" class="supplier_buy" id="BuySupplierId"><option value="0">-- Pilih Supplier --</option>';
        $order_buy .= $this->generatedActiveSuppliers($suppliers, $b);
        $order_buy .= '</select></div>';
        $order_buy .= $this->input('Buy.name.', array(
                    'type' => 'text',
                    'class' => 'bn',
                    'label' => 'Tipe',
                    'placeholder' => 'Tipe model Barang',
                    'value' => $set ? $b['name'] : ''
                        )
                ) . $this->input('Buy.bahan.', array(
                    'type' => 'text',
                    'class' => 'bb',
                    'label' => 'Bahan & Warna',
                    'placeholder' => 'Bahan dan Warna',
                    'value' => $set ? $b['bahan'] : ''
                        )
                ) . $this->input('Buy.sell_price.', array(
                    'type' => 'text',
                    'label' => 'Harga Jual / Unit',
                    'class' => 'auto',
                    'placeholder' => 'Rp',
                    'value' => $set ? 'Rp ' . number_format($b['sell_price']) : ''
                        )
                ) . $this->input('Buy.list_price.', array(
                    'type' => 'text',
                    'label' => 'Pricelist / Unit',
                    'class' => 'auto blp',
                    'placeholder' => 'Rp',
                    'value' => $set ? 'Rp ' . number_format($b['list_price']) : ''
                        )
                ) . $this->input('Buy.supplier_discount.', array(
                    'type' => 'hidden',
                    'label' => 'Discount',
                    'class' => 'disc',
                    'placeholder' => '%',
                    'value' => $set ? number_format($b['supplier_discount']) : ''
                        )
                ) . $this->input('Buy.stock_id.', array(
                    'type' => 'hidden',
                    'label' => 'stock_id',
                    'class' => 'id',
                    'placeholder' => '',
                    'value' => $set ? $b['stock_id'] : ''
                        )
                ) . $this->input('Buy.product_id.', array(
                    'type' => 'hidden',
                    'label' => 'product_id',
                    'class' => 'pro_id',
                    'placeholder' => '',
                    'value' => $set ? $b['product_id'] : ''
                        )
        );

        if (isset($showDiscount) && $showDiscount == true) {
            $order_buy .= $this->input('Buy.supplier_discount.', array(
                'type' => 'text',
                'class' => 'auto',
                'label' => 'Diskon dari Supplier (%)',
                'value' => $set ? $b['supplier_discount'] : ''
                    )
            );
        }
        $order_buy .= '<div style="margin-top:25px;"><button type="button" id="but_stock" class="btn btn-warning stbu" data-toggle="modal" data-target="#myModal" style="float:left;">From Stock</button></div>';
        $order_buy .= '</div>';
        return $order_buy;
    }
    ...
    /**
     * Upgrade_order.ctp HTML
     * Placed here because you can't define functions in view
     */
    function orderBuyHtml($suppliers, $showDiscount = false, $b = null, $new = FALSE, $change_col = NULL) {
        $set = isset($b) && is_array($b);
        $close_btn = $set;
        $set = TRUE;
        if ($change_col === NULL) {
            $change_col = array('Buy' => array());
            $close_btn = FALSE;
        }
        if ($new === TRUE) {
            $close_btn = FALSE;
        }
        $new = ($new === TRUE) ? ' style="background-color:#f0ad4e;"' : '';

        $order_buy = '
            <div class="order_buy"' . $new . '>'
                . ($close_btn ? '' : '<a href="#" class="remove_order_buy">x</a>')
                . ($set ? $this->input('Buy.id.', array('type' => 'hidden', 'value' => (isset($b['id'])) ? $b['id'] : NULL)) : '')
                . $this->input('Buy.qty.', array(
                    'type' => 'number',
                    'label' => 'Jumlah ' . $this->markChange($change_col, 'qty'),
                    'style' => 'width:60px;' . $this->diffRed('qty', $change_col),
                    'value' => $set ? $b['qty'] : ''))
                . $this->input('Buy.supplier_id.', array(
                    'options' => $suppliers,
                    'type' => 'select',
                    'label' => 'Supplier ' . $this->markChange($change_col, 'supplier_id'),
                    'empty' => 'Pilih',
                    'class' => 'supplier_buy',
                    'value' => $set ? $b['supplier_id'] : '',
                    'style' => $this->diffRed('supplier_id', $change_col)))
                . $this->input('Buy.name.', array(
                    'type' => 'text',
                    'label' => 'Tipe ' . $this->markChange($change_col, 'name'),
                    'placeholder' => 'Tipe model Barang',
                    'value' => $set ? $b['name'] : '',
                    'style' => $this->diffRed('name', $change_col)))
                . $this->input('Buy.bahan.', array(
                    'type' => 'text',
                    'label' => 'Bahan & Warna ' . $this->markChange($change_col, 'bahan'),
                    'value' => $set ? $b['bahan'] : '',
                    'style' => $this->diffRed('bahan', $change_col)))
                . $this->input('Buy.sell_price.', array(
                    'type' => 'text',
                    'label' => 'Harga Jual / Unit ' . $this->markChange($change_col, 'sell_price'),
                    'class' => 'auto',
                    'placeholder' => 'Rp',
                    'value' => ($set AND $b['sell_price']) ? 'Rp ' . number_format($this->filterPrice($b['sell_price'])) : 0,
                    'style' => $this->diffRed('sell_price', $change_col)))
                . $this->input('Buy.list_price.', array(
                    'type' => 'text',
                    'label' => 'Pricelist / Unit ' . $this->markChange($change_col, 'list_price'),
                    'class' => 'auto',
                    'placeholder' => 'Rp',
                    'value' => ($set AND $b['list_price']) ? 'Rp ' . number_format($this->filterPrice($b['list_price'])) : 0,
                    'style' => $this->diffRed('list_price', $change_col)));
        if (isset($showDiscount) && $showDiscount == true) {
            $order_buy .= $this->input('Buy.supplier_discount.', array(
                'type' => 'text',
                'label' => 'Diskon dari Supplier (%) ' . $this->markChange($change_col, 'supplier_discount'),
                'value' => $set ? $b['supplier_discount'] : '',
                'style' => $this->diffRed('supplier_discount', $change_col)));
        }
        $order_buy .= '</div>';
        return $order_buy;
    }
    ...
    function orderBuyQuoteHtml($suppliers, $showDiscount = false, $b = null, $new = FALSE, $change_col = NULL) {
        $set = isset($b) && is_array($b);
        $close_btn = $set;
        $set = TRUE;
        if ($change_col === NULL) {
        ...
        $order_buy = '
            <div class="order_buy"' . $new . '>'
                ...
                . $this->input('Buy.sell_price.', array(
                    'type' => 'text',
                    'label' => 'Harga Jual / Unit ' . $this->markChange($change_col, 'price'),
                    'class' => 'auto',
                    'placeholder' => 'Rp',
                    'value' => ($set AND $b['price']) ? 'Rp ' . number_format($this->filterPrice($b['price'] * (1 - ($this->calculateTotalDiscount($b['disc']) / 100)))) : 0,
                    'style' => $this->diffRed('sell_price', $change_col)))
                . $this->input('Buy.list_price.', array(
                    'type' => 'text',
                    'label' => 'Pricelist / Unit ' . $this->markChange($change_col, 'price'),
                    'class' => 'auto',
                    'placeholder' => 'Rp',
                    'value' => ($set AND $b['price']) ? 'Rp ' . number_format($this->filterPrice($b['price'])) : 0,
                    'style' => $this->diffRed('list_price', $change_col)))
                . $this->input('Buy.product_id.', array(
                    'type' => 'hidden',
                    'label' => 'Product Id ',
                    ...
                    'placeholder' => '',
                    'value' => $set ? $b['product_id'] : '',
                    'style' => $this->diffRed('product_id', $change_col)));
        if (isset($showDiscount) && $showDiscount == true) {
            $order_buy .= $this->input('Buy.supplier_discount.', array(
                'type' => 'text',
                'label' => 'Diskon dari Supplier (%) ' . $this->markChange($change_col, 'supplier_discount'),
    ...
    function quoteListHtml($suppliers, $showDiscount = false, $b = null) {
        $set = isset($b) && is_array($b);

        $quote_list = '<tr>';
        if (isset($b['id'])) {
            $quote_list .= $this->input('Quote.id.', array('type' => 'hidden', 'value' => $b['id']));
        }
        $quote_list .= $this->input('Quote.qty.', array('type' => 'number', 'label' => 'Jumlah', 'value' => $set ? $b['qty'] : '', 'div' => 'col-sm-2'));
        $quote_list .= '<div class="input select required">
                        <label for="QuoteSupplierId">Supplier</label>
                        <select name="data[Quote][supplier_id][]" class="supplier_buy"><option value="0">-- Pilih Supplier --</option>';
        $quote_list .= $this->generatedActiveSuppliers($suppliers, $b);
        $quote_list .= '</select>'
                . '</div>';
        $quote_list .= $this->input('Quote.item.', array('type' => 'text', 'label' => 'Item', 'value' => $set ? $b['item'] : ''));
        $quote_list .= $this->input('Quote.bahan.', array('type' => 'text', 'label' => 'Jenis Barang', 'value' => $set ? $b['bahan'] : ''));
        $quote_list .= $this->input('Quote.price.', array('type' => 'text', 'placeholder' => 'Rp', 'label' => 'Harga', 'class' => 'auto', 'value' => $set ? $b['price'] : ''));
        $quote_list .= $this->input('Quote.disc.', array('type' => 'text', 'placeholder' => '%', 'label' => 'Disc (%)', 'value' => $set ? $b['disc'] : ''));
        $quote_list .= '<a data-key="' . $b['id'] . '" href="#" class="remove_quote_item"><i class="fa fa-trash"></i></a>';

        $quote_list .= '</tr>';
        return $quote_list;
    }

    function ProjectListHtml($b = null) {
        $set = isset($b) && is_array($b);

        $list = '<div class="quote_list">' . ($set ? '<a href="#" class="remove_quote_item">x</a>' : '<a href="#" class="remove_quote_item">x</a>');
        if (isset($b['id'])) {
            $list .= $this->input('Parameter.id.', array('type' => 'hidden', 'value' => $b['id']));
        }
        $list .= $this->input('Parameter.name.', array('type' => 'text', 'label' => 'Point Penilaian', 'value' => $set ? $b['name'] : ''))
                . $this->input('Parameter.value.', array('type' => 'text', 'label' => 'Nilai(0-1)', 'value' => $set ? $b['value'] : ''));
        $list .= '</div>';
        return $list;
    }

    function paymentListHtml($b = null) {
        $set = isset($b) && is_array($b);
        $quote_list = '<div class="quote_list row">'
                . $this->input('Payment.amount.', array('type' => 'text', 'label' => 'Jumlah', 'class' => 'form-control auto required',
                    'div' => 'input text required col-sm-3', 'value' => $set ? $b['amount'] : ''))
                . $this->input('Payment.keterangan.', array('class' => 'form-control', 'type' => 'text', 'div' => 'col-sm-6 input text required', 'label' => 'Keterangan', 'value' => $set ? $b['description'] : ''))
                . ($set ? '' : '<div class="col-sm-3"><br/><a href="#" class=" btn btn-danger remove_quote_item"><i class="fa fa-trash-o"></i></a></div>');
        $quote_list .= '</div>';
        return $quote_list;
    }

    function LeaveListHtml($types, $b = null) {
        $set = isset($b) && is_array($b);
        $quote_list = '<div class="quote_list">'
                . ($set ? '' : '<a href="#" class="remove_quote_item">x</a>')
                . ($set ? $this->input('LeavesTaken.id.', array('type' => 'hidden', 'value' => $b['id'])) : '')
                . $this->input('LeavesTaken.type_id.', array('options' => $types,
                    'type' => 'select', 'label' => 'Type', 'class' => 'supplier_buy'
                    , 'value' => $set ? $b['type_id'] : ''))
                . $this->input('LeavesTaken.tanggal.', array('id' => '', 'type' => 'text', 'label' => 'Tanggal', 'class' => 'dpx', 'value' => $set ? $b['item'] : ''));
        $quote_list .= '</div>';
        return $quote_list;
    }

    function KasbonListHtml($b = null) {
        $bayar_pake = array("pokok" => "Gaji Pokok", 'bonus' => "Bonus");
        $yperiode = array(date("Y") => date("Y"), date("Y") + 1 => date("Y") + 1);
        $mperiode = array('01' => "Januari", '02' => "Februari", "03" => "Maret", "04" => "April", "05" => "Mei", "06" => "Juni", "07" => "Juli", "08" => "Agustus", "09" => "September", "10" => "Oktober", "11" => "November", "12" => "Desember");
        $set = isset($b) && is_array($b);
        $quote_list = '<div class="quote_list">'
                . ($set ? '' : '<a href="#" class="remove_quote_item">x</a>')
                . $this->input('KasbonPayment.yperiode.', array('options' => $yperiode, 'type' => 'select', 'class' => 'yperiode', 'label' => 'Periode', 'div' => 'input text required', 'value' => $set ? $b['yperiode'] : ''))
                . $this->input('KasbonPayment.mperiode.', array('options' => $mperiode, 'type' => 'select', 'class' => 'mperiode', 'label' => 'Periode', 'div' => 'input text required', 'value' => $set ? $b['mperiode'] : ''))
                . $this->input('KasbonPayment.bayar_pake.', array('options' => $bayar_pake, 'type' => 'select', 'label' => 'Bayar Menggunakan', 'div' => 'input text required', 'value' => $set ? $b['bayar_pake'] : ''))
                . $this->input('KasbonPayment.amount.', array('type' => 'text', 'class' => 'auto', 'div' => 'input text required', 'label' => 'Jumlah', 'value' => $set ? $b['amount'] : ''));
        $quote_list .= '</div>';
        return $quote_list;
    }

    function OrderListHtml($orders, $b = null) {
        $set = isset($b) && is_array($b);
        $quote_list = '<div class="quote_list">'
                . ($set ? '' : '<a href="#" class="remove_quote_item">x</a>')
                . $this->input('Payment.order_id.', array('options' => $orders,
                    'type' => 'select', 'label' => 'Untuk Pembayaran Order', 'empty' => 'Pilih', 'class' => 'span6 supplier_buy chosen'
                    , 'value' => $set ? $b['order_id'] : ''))
                . $this->input('Payment.amount.', array('type' => 'text', 'label' => 'Amount', 'class' => 'auto', 'div' => 'input text required', 'value' => $set ? $b['amount'] : ''));
        $quote_list .= '</div>';
        return $quote_list;
    }

    function PayGiroListHtml($b = null) {
        $set = isset($b) && is_array($b);
        $quote_list = '<div class="quote_list">'
                . ($set ? '' : '<a href="#" class="remove_quote_item">x</a>')
                . $this->input('Payment.no_seri.', array('type' => 'text', 'label' => 'Nomor Giro', 'value' => $set ? $b['no_seri'] : ''))
                . $this->input('Payment.amount.', array('type' => 'text', 'label' => 'Amount', 'class' => 'auto', 'div' => 'input text required', 'value' => $set ? $b['amount'] : ''))
                . $this->input('Payment.due_date.', array('type' => 'text', 'label' => 'Tanggal Cair', 'class' => 'date', 'id' => '', 'div' => 'input text', 'value' => $set ? $b['due_date'] : ''));
        $quote_list .= '</div>';
        return $quote_list;
    }
     ...
     * OpenLead special
     */
    function openSuppliersHtml($suppliers, $osupp = null) {
        $set = isset($osupp) && is_array($osupp);

        $orders_suppliers = '<div class="order_supplier">'
                . ($set ? $this->input('OrderSupplier.id.', array(
                    'type' => 'hidden',
                    'value' => $osupp['id'])
                ) : '');

        $orders_suppliers .= '<div class="input select required">' .
                '<label for="OrderSupplierSupplierId">Supplier </label>' .
                '<select name="data[OrderSupplier][supplier_id][]" class="supplier_order" id="OrderSupplierSupplierId">';
        ...
        foreach ($suppliers as $sup) {
            ...
            $selected = ($osupp['supplier_id'] == $sup['Supplier']['id']) ? 'selected="selected"' : '';
            $orders_suppliers .= '<option ' . $selected . ' value="' . $sup['Supplier']['id'] . '" ' . $_style . '>' . $sup['Supplier']['name'] . '</option>';
            unset($_style);
        }
        $orders_suppliers .= '</select></div>';


        $orders_suppliers .= $this->input('OrderSupplier.latest_delivery_date.', array('type' => 'text'
                    , 'class' => 'datepicker', 'placeholder' => 'DD-MM-YYYY'
                    , 'label' => 'Tanggal Pengiriman '
                    , 'div' => 'input text required'
                    , 'value' => $set ? date('d-m-Y', strtotime($osupp['latest_delivery_date'])) : ''
                    , 'id' => ''))
                . $this->input('OrderSupplier.dp.', array(
                    'type' => 'text'
                    , 'label' => 'DP yang diperlukan '
                    , 'class' => 'auto', 'placeholder' => 'Rp', 'div' => 'input text required'
                    , 'value' => $set ? $osupp['dp'] : ''))
                . ($set ? '' : '<a href="#" class="remove_order_supplier">x</a>')
                . '</div>';
        return $orders_suppliers;
    }

    function ordersSuppliersHtml($suppliers, $osupp = null, $new = FALSE, $change_col = NULL) {
        $set = isset($osupp) && is_array($osupp);
        $close_btn = $set;
        $new = ($new === TRUE) ? ' style="background-color:#f0ad4e;"' : '';
        if ($change_col === NULL) {
            $change_col = array('OrdersSupplier' => array());
            $close_btn = TRUE;
        }
        ...
        $orders_suppliers = '<div class="order_supplier"' . $new . '>'
                . ($set ? $this->input('OrderSupplier.id.', array(
                    'type' => 'hidden',
                    'value' => $osupp['id'])
                ) : '')
                . $this->input('OrderSupplier.supplier_id.', array(
                    'options' => $suppliers
                    , 'style' => $this->diffRed('supplier_id', $change_col)
                    , 'type' => 'select'
                    , 'label' => 'Supplier ' . $this->markChange($change_col, 'supplier_id')
                    ...
                    , 'value' => $set ? $osupp['supplier_id'] : ''))
                . $this->input('OrderSupplier.latest_delivery_date.', array('type' => 'text'
                    , 'class' => 'datepicker', 'placeholder' => 'DD-MM-YYYY'
                    , 'label' => 'Tanggal Pengiriman ' . $this->markChange($change_col, 'latest_delivery_date')
                    , 'style' => $this->diffRed('latest_delivery_date', $change_col)
                    , 'div' => 'input text required'
                    , 'value' => $set ? date('d-m-Y', strtotime($osupp['latest_delivery_date'])) : ''
                    , 'id' => ''))
                . $this->input('OrderSupplier.dp.', array(
                    'type' => 'text',
                    'label' => 'DP yang diperlukan ' . $this->markChange($change_col, 'dp')
                    , 'style' => $this->diffRed('dp', $change_col)
                    , 'class' => 'auto', 'placeholder' => 'Rp', 'div' => 'input text required'
                    , 'value' => $set ? $osupp['dp'] : ''))
                . '<a data-key="4" href="#" class="remove_order_supplier">x</a>'
                . '</div>';
        return $orders_suppliers;
    }

    protected function generateOpenShippers($suppliers, $shippers, $osupp = NULL) {
        $shipper_id = ( isset($osupp['shipper_id']) ) ? $osupp['shipper_id'] : '';
        $tmp = '';
        foreach ($shippers as $key => $sp) {
            $selected = ($shipper_id == $key) ? 'selected="selected"' : '';
            $tmp .= '<option ' . $selected . ' value="shipper_' . $key . '">Ekspedisi : ' . $sp . '</option>';
        }

        $supplier_id = ( isset($osupp['supplier_id']) ) ? $osupp['supplier_id'] : '';
        foreach ($suppliers as $sup) {
            if ((int) $sup['Supplier']['status'] <= 0) {
                $_style = 'style="background-color:#DDD;" disabled="disabled"';
            } else
            ...
            $selected = ($supplier_id == $sup['Supplier']['id']) ? 'selected="selected"' : '';
            $tmp .= '<option ' . $selected . ' value="supplier_' . $sup['Supplier']['id'] . '" ' . $_style . '>' . $sup['Supplier']['name'] . '</option>';
            unset($_style);
        }
    ...
    function openShippingsHtml($shippers, $shipping = null, $suppliers, $change_col = NULL) {
        $set = isset($shipping) && is_array($shipping);
        $close_btn = $set;
        if ($change_col === NULL) {
            $change_col = array('Shipping' => array());
        }

        $order_shipping = '<div class="order_shipping">'
                . ($set ? $this->input('Shipping.id.', array('type' => 'hidden', 'value' => $shipping['id'])) : '');
        /* . $this->input('Shipping.shipper_supplier_id.', array(
          'options' => $shippers,
          'type' => 'select'
          ,'style' => $this->diffRed('shipper_supplier_id',$change_col)
          ,'label' => 'Pengirim '.$this->markChange($change_col,'shipper_supplier_id')
          ,'empty' => 'Pilih',
          'class' => 'shipping_order'
          , 'value' => $set ? $shipping['shipper_supplier_id'] : '')) */
        $order_shipping .= '<div class="input select required">
                        <label for="ShippingShipperSupplierId">Pengirim ' . $this->markChange($change_col, 'shipper_supplier_id') . '</label>
                        <select name="data[Shipping][shipper_supplier_id][]" class="shipping_order" id="ShippingShipperSupplierId">';
        $order_shipping .= '<option>Pilih</option>';
        $order_shipping .= $this->generateOpenShippers($suppliers, $shippers, $shipping);
        $order_shipping .= '</select></div>';

        $order_shipping .= $this->input('Shipping.sell_price.', array(
                    'type' => 'text'
                    , 'style' => $this->diffRed('sell_price', $change_col)
                    , 'label' => 'Harga ke Customer ' . $this->markChange($change_col, 'sell_price')
                    , 'class' => 'auto'
                    , 'placeholder' => 'Rp'
                    , 'div' => 'input text required'
                    , 'value' => $set ? $shipping['sell_price'] : ''))
                . $this->input('Shipping.buy_price.', array(
                    'type' => 'text'
                    , 'style' => $this->diffRed('buy_price', $change_col)
                    , 'label' => 'Modal Pengiriman ' . $this->markChange($change_col, 'buy_price')
                    , 'class' => 'auto', 'placeholder' => 'Rp', 'div' => 'input text required'
                    , 'value' => $set ? $shipping['buy_price'] : ''))
                . ($close_btn ? '' : '<a href="#" class="remove_order_shipping">x</a>')
                . '</div>';
        return $order_shipping;
    }

    function ordersShippingsHtml($shippers, $shipping = null, $new = FALSE, $change_col = NULL) {
        $set = isset($shipping) && is_array($shipping);
        $close_btn = $set;
        if ($change_col === NULL) {
            $change_col = array('Shipping' => array());
        ...
        $order_shipping = '<div class="order_shipping">'
                ...
                . $this->input('Shipping.shipper_supplier_id.', array(
                    ...
                    'class' => 'shipping_order'
                    , 'value' => $set ? $shipping['shipper_supplier_id'] : ''))
                . $this->input('Shipping.sell_price.', array(
                    'type' => 'text'
                    , 'style' => $this->diffRed('sell_price', $change_col)
                    , 'label' => 'Harga ke Customer ' . $this->markChange($change_col, 'sell_price')
                    , 'class' => 'auto'
    ...
    public function drop_down_from_threaded($data_array, $options) {
        $result = '';

        foreach ($data_array as $data) {
            if (is_array($options))
                $selected = ( $options['default'] == $data['Coa']['id'] ) ? 'selected="selected"' : '';
            else
                $selected = '';

            $result .= '<option ' . $selected . ' value="' . $data['Coa']['id'] . '">' . $data['Coa']['code'] . ' - ' . $data['Coa']['name'] . '</option>';
            if (count($data['children']) > 0) {
                $result .= $this->drop_down_from_threaded($data['children'], $options);
            }
        }
    ...
    //entry journal drop_down rekening
    public function drop_down_entry($data_array, $options) {
        $result = '';
        ...
        foreach ($data_array as $data) {
            ...
            if ((int) $data['Coa']['tipe_rekening'] === 0) {
                $result .= '<optgroup label="' . $data['Coa']['code'] . ' - ' . $data['Coa']['name'] . '">';
            } else {
                $result .= '<option ' . $selected . ' value="' . $data['Coa']['id'] . '">' . $data['Coa']['code'] . ' - ' . $data['Coa']['name'] . '</option>';
            }

            if (count($data['children']) > 0) {
                $result .= $this->drop_down_entry($data['children'], $options);
            }
    ...
    public function usersSelect($users) {
        if (!is_array($users))
    ...
    //generate select bank
    public function banks($sources, $default = NULL, $option = NULL) {
        $tmp = '<div class="required pull-left required">';
        $tmp .= '<label for="PaymentBank">Bank</label>';
        $tmp .= '<select id="PaymentBank" name="data[Payment][bank]">';
        foreach ($sources as $src) {
            $default_selected = (strtolower($default) === strtolower($src['Bank']['short_name'])) ? 'selected="selected"' : '';
            if ($option === 'inout') {
                if ($src['Bank']['bank_type'] === 'in')
                    continue;
                $tmp .= '<option ' . $default_selected . ' value="' . $src['Bank']['short_name'] . '">' . $src['Bank']['nama_bank'] . '</option>';
            }elseif ($option === 'in') {
    ...
    /**
     *   Mark Field Input with red background
     */
    public function diffRed($key, $change_col) {
        $_tmp = '';
        if (!is_array($change_col))
            if ($change_col === '*') {
        ...
        if ($this->Array->Get($change_col, $key) !== NULL) {
            $_tmp = 'background-color:#f0ad4e;';
        }
    ...
    /**
     * Convert Rp 20,500.00 to 20500.00
     *
     * @param type $price
     * @return type
     */
